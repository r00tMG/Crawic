...
class FlaskApiDomainExpiredService
{
    ...
    public function getData($page = 1, $perPage = 5000)
    {
        $queryParams = [
            'page' => $page,
            'per_page' => $perPage,
        ...
        ->get("{$this->apiBaseUrl}/get_deleted_domains", $queryParams);
        ...
        if ($response->successful()) {
            //$filePath = storage_path('app/get_deleted_domains.json');
    ...
    /* public function getData()
    {
        ...
        $response = Http::get("{$this->apiBaseUrl}/get_deleted_domains"); 
        print_r($response->successful());
        if ($response->successful()) {
            $data = $response->json();
            ////print_r("data: " . $data);
            return $data;
            #return $this->filterDomains($data);
        }
    ...
    private function filterDomains($domains)
    {
        ...
        foreach ($domains as $domain) {
            ...
            //print_r("ration rd/backlinks: " . $rdRatio);
            
            // Vérifier si le domaine est indexé dans Google
            $isIndexed = $this->checkGoogleIndexing($domain['domain']);
            //print_r("isIndexed: " . $isIndexed);
            
            // Vérifier le contenu suspect (caractères asiatiques, etc.)
            $hasSuspiciousContent = $this->checkSuspiciousContent($domain['domain']);
            logger("domain name: " . $domain['domain'] . " ration rd/backlinks: " . $rdRatio . " isIndexed: " . $isIndexed . " hasSuspiciousContent: " . $hasSuspiciousContent);

            //print_r("hasSuspiciousContent: " . $hasSuspiciousContent);
            if ($rdRatio >= 0.05 && $isIndexed && !$hasSuspiciousContent) {
                $filteredDomains[] = $domain;
            }
        }
        //print_r("filteredDomains: " . $filteredDomains);
    ...
    public function assignCategories(array $domain): array
    {
        ...
        foreach ($categoryKeywords as $pattern => $categoryId) {
            if (preg_match("/$pattern/i", $domain['domain'])) {
